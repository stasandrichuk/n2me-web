...
// Custom Scripts For Medianet API
// ========================================================= //

$('#search').on('keypress', function(event) {
    if($("#search").val() == '' || event.keyCode != 13) return;
    $('#music_search_form').submit();  
});

$('label.search-ico').on('click', function(event) {
    if($("#search").val() == '') return;
    $('#music_search_form').submit();  
});

function draw_albums(t_list) {
    var track_html = '';
    t_list["Tracks"].forEach(function(track) {
        track_html = track_html + '<div class="album" data-id="' + track['MnetId'] + '">';
        
        if (!("Album" in track)) {
            track_html = track_html + '<div class="art"><p>No Album</p></div>';                        
        } else {
            track_html = track_html + '<div class="art"><img src="' + track['Album']['Images']['Album800x800'] + '"></div>';
        }   
        
        track_html = track_html + '<div class="info"><h1>' + track['Title'] + '</h1><p>' + track['Artist']['Name'] + '</p></div>';

        track_html = track_html + '</div>';
    });   

    return track_html;
}

function get_more_musics() {
    return $.ajax({
        url: "music/more_musics?page=" + $('#page').val() + "&title=" + gon.title,
        dataType : "json",
        type: "GET",
        success: function(data, event, status, xhr) {
            if(data.success == true) {
                var track_list = data.track_list;
                $('.content .tab.results').addClass('active');

                var track_html = draw_albums(track_list);

                if(data.more_flag == true) {
                    $(track_html).insertBefore('.tab.results .more-btn');   
                } else {
                    $(track_html).insertBefore('.tab.results .more-btn'); 
                    $('.tab.results .more-btn').remove();    
                }
...
$(window).on('load', function() {
    if(gon.success == true) {
        $('.content.discover .tab.topsongs').removeClass('active');
        var track_list = gon.track_list;
        $('.content .tab.results').addClass('active');

        $('<input type="hidden" name="page" id="page"></input>').appendTo('.content .tab.results');
        
        var track_html = draw_albums(track_list);

        if(gon.more_flag == true) {
            track_html = track_html + '<div class="more-btn"><a class="btn btn-info"><span>MORE </span><i class="fa fa-arrow-down fa-2x"></i></a></div>';
        }
        
        $(track_html).appendTo('.content .tab.results');

        if(typeof gon.title != 'undefined') $('#search').val(gon.title);

        $('#page').val(gon.page);

        $('.tab.results .more-btn a').on('click', get_more_musics);

        $('#bottomArt').html('<img src="' + track_list["Tracks"][0]['Album']['Images']['Album800x800'] + '">');
    }
});
